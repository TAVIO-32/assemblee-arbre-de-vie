/**
 * routes/cotisations.js — Cotisations (multi-tenant ZAURA).
 */
...
router.get('/me', async (req, res) => {
  const cotisations = await db.all(
    'SELECT id, montant, libelle, date, statut FROM cotisations WHERE membre_id = ? ORDER BY date DESC', req.user.id);
...
router.get('/', requireEncadrement, async (req, res) => {
  let sql = `
    ...
    FROM cotisations c JOIN users u ON u.id = c.membre_id LEFT JOIN tribus t ON t.id = u.tribu_id
    WHERE c.org_id = ?`;
...
router.post('/', requireEncadrement, async (req, res) => {
  ...
  if (!membre_id || !Number.isFinite(m) || m <= 0 || !date) {
    return res.status(400).json({ error: 'Fidele, montant positif et date obligatoires.' });
  }
  if (!(await peutAccederMembre(req, membre_id))) return res.status(403).json({ error: 'Acces refuse.' });
  const id = await db.insert(`
    INSERT INTO cotisations (org_id, membre_id, montant, libelle, date, statut, enregistre_par)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `, req.org_id, Number(membre_id), m, String(libelle || '').trim(), String(date).slice(0, 10),
     statut === 'paye' ? 'paye' : 'non_paye', req.user.id);
  res.status(201).json({ id });
});

async function chargerCotisation(req, res) {
  const c = await db.get('SELECT * FROM cotisations WHERE id = ? AND org_id = ?', Number(req.params.id), req.org_id);
  if (!c) { res.status(404).json({ error: 'Cotisation introuvable.' }); return null; }
  if (!(await peutAccederMembre(req, c.membre_id))) { res.status(403).json({ error: 'Acces refuse.' }); return null; }
  return c;
}
